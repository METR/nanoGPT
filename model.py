...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = self.c_attn(x_local).split(self.n_embd, dim=2)
        k = k.view(B, local_T, self.n_head, C // self.n_head).transpose(1, 2).contiguous() # (B, nh, local_T, hs)
        ...
        if self.flash:
            ...
            causal_mask = self.bias[:,:,:T,:T].to(att.device)
            att = att.masked_fill(causal_mask[:,:,start_idx:end_idx,:] == 0, float('-inf'))
            att = F.softmax(att, dim=-1)
            att = self.attn_dropout(att)
            y = att @ v_all # (B, nh, local_T, hs)

        y = y.transpose(1, 2).contiguous().view(B, local_T, C) # re-assemble all head outputs side by side
